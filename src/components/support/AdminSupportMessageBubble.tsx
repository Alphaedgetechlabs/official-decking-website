...
import type { AdminSupportMessage } from '../../types/adminSupportChat';
import { formatMessageTime } from '../../utils/formatRelativeTime';
import { MediaFullscreenViewer } from '../messages/MediaFullscreenViewer';
import { ChatMediaImageCard, ChatMediaVideoCard } from '../messages/ChatMediaImageCard';
import { AdminSupportMessageStatusIcon } from './AdminSupportMessageStatusIcon';

interface AdminSupportMessageBubbleProps {
  message: AdminSupportMessage;
  onAdminMessageVisible?: (messageId: string) => void;
}

export function AdminSupportMessageBubble({
  message,
  onAdminMessageVisible,
}: AdminSupportMessageBubbleProps) {
  ...
  const canMarkSeen =
    ...
    Boolean(onAdminMessageVisible);

  useEffect(() => {
    markedSeenRef.current = message.status === 'seen';
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (!entry?.isIntersecting || markedSeenRef.current) return;

        markedSeenRef.current = true;
        onAdminMessageVisible?.(message.id);
      },
      { threshold: 0.75 },
    );
  ...
  }, [canMarkSeen, message.id, onAdminMessageVisible]);
      ...
      <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
        ...
        >
          ...
          >
            ...
            {isOwn && <AdminSupportMessageStatusIcon status={message.status} />}
          </div>
        </div>
      </div>
